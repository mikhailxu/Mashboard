 ...
 *
 * with mousewheel hack by MSG
 * with swipe detection, too.
 * 
 */
...
function iScroll (el, options) {
	...
	// Find swipe hashes
	var swipeLink = this.element.querySelector('.swipeleft');
	if (swipeLink)
	{
		this.swipeLeft = swipeLink.hash.substr(1);
	}
	swipeLink = this.element.querySelector('.swiperight');
	if (swipeLink)
	{
		this.swipeRight = swipeLink.hash.substr(1);
	}
	...
	this.wrapper.addEventListener('mousewheel', this);	// Hack to make mousewheel scroll too
...
iScroll.prototype = {
	...
	onTouchStart: function(e) {
		...
		// For swipe detection we have to keep another startX and startY
		// since touchStartX and touchStartY are overwritten in onTouchMove
	...
	onTouchEnd: function(e) {
		...
		if (!this.scrollX)	// no panning in x dimension, so implement swipe
		{
			var xdelta = this.swipeStartX - this.touchStartX;
			var ydelta = this.swipeStartY - this.touchStartY;
			var xd = Math.abs(xdelta);
			var yd = Math.abs(ydelta);
			if ((xd > yd) && (xd > 50) && ((e.timeStamp - this.swipeStartTime) < 500))
			{
				// let's call it a swipe
				if ((xdelta < 0) && this.swipeRight)
				{
					iui.replacePage(this.swipeRight, /*backwards*/ true);
				}
				else if ((xdelta > 0) && this.swipeLeft)
				{
					iui.replacePage(this.swipeLeft);
				}
			}
		}
	...
	// onMouseWheel: Very buggy, but good enough to do some testing in Desktop Safari!
	onMouseWheel: function(e) {
		...
		var dist = this.y - this.scrollStartY;
		var maxYDist1 =  -this.y + /*this.scrollHeight/3*/ 50;
		var maxYDist2 = this.y + this.element.offsetHeight - this.scrollHeight + /*this.scrollHeight/3*/ 50;
		var newDist = dist + rolled;
		if (dist > 0 && newDist > maxYDist1) {
			newDist = maxDist1;
		}
		if (dist < 0 &&  newDist > maxDist2) {
			newDist = maxDist2;
		}
		
		
		this.scrollTo(this.x, this.y + rolled, '10ms');
		console.log("mouseWheel rolled: " + rolled);
	},
